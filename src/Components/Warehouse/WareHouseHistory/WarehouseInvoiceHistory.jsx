...
export default function WarehouseInvoiceHistory() {
    ...
    const initialPage = Number(searchParams.get("page")) || Number(localStorage.getItem(LS_KEYS.PAGE)) || 1;
    ...
    const [statusesList] = useState(["draft", "confirmed", "received", "cancelled"]);
                ...
                <div className="bg-white p-4 rounded-2xl border mb-6 shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div className="flex gap-2">
                            <input value={searchText} onChange={(e) => { setSearchText(e.target.value); setPage(1); }} placeholder="Search id / barcode / batch" className="flex-1 border rounded-lg p-2" />
                            <button onClick={() => { setTypeFilter(""); setSenderFilter(""); setReceiverFilter(""); setStatusFilter(""); setPaymentFilter(""); setFromDate(""); setToDate(""); setSearchText(""); setPage(1); }} className="px-3 py-2 bg-gray-100 rounded-lg">Reset</button>
                        ...
                        <div>
                                    ...
                                    <tbody>
                                        {invoices.map((inv) => (
                                            <tr key={inv.id} className="border-b hover:bg-gray-50">
                                                {columns.id && <td className="py-3 px-2 text-sm">{inv.id.slice(0, 8)}</td>}
                            ...
                            <div className="mt-4 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-2 py-1 border rounded"><ChevronLeft size={16} /></button>
                                    <div className="px-3 py-1 border rounded">{page} / {totalPages}</div>
                                    <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-2 py-1 border rounded"><ChevronRight size={16} /></button>
