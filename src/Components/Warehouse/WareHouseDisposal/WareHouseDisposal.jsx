// src/Components/Warehouse/WareHousePages/WareHouseDisposal.jsx
...
export default function WareHouseDisposal() {
    ...
    // context (per-mode)
    const {
        mode, // should be "dis" from WarehouseLayout
        mixData,
        addItem,
        updateQty,
        updatePrice,
    ...
    const toggleSidebar = () => {
        if (!invoiceStarted?.[mode]) {
            notify.info("Kategoriya panelini ochish uchun utilizatsiyani boshlang");
            return;
        }
    ...
    const fetchLocations = async () => {
        try {
            ...
            if (res?.status === 200 || res?.status === 201) {
                ...
                const disLoc = (res.data || []).find((l) => l.type === "disposal");
                if (disLoc) setDisposalLocation(disLoc);
            } else {
                setLocations(res || []);
            }
        } catch (err) {
    ...
    // update invoiceMeta sender for disposal: sender is current location name, receiver is disposal location name
    useEffect(() => {
        const senderName = getLocationNameById(userLId) || "Me";
        const receiverName = disposalLocation?.name || "Disposal";
        setInvoiceMeta(mode, { ...invoiceMeta?.[mode], sender: senderName, receiver: receiverName });
    ...
    const handleCategoryClick = async (catId) => {
        ...
        try {
            ...
            // for disposal: fetch stocks of current location (we will select items from current location)
    ...
    // start disposal invoice
    const startInvoice = async () => {
        // for disposal: receiver_id is disposal location id
        const receiver = disposalLocation?.id;
        if (!receiver) {
            notify.error("Disposal location topilmadi. Iltimos locationlarni tekshiring");
            return;
        }
        try {
            ...
            if (res?.status === 200 || res?.status === 201) {
                ...
                setInvoiceMeta(mode, { ...invoiceMeta?.[mode], sender: getLocationNameById(userLId), receiver: disposalLocation?.name || "Disposal" });
                setIsDirty(mode, false);
                notify.success("Utilizatsiya boshlandi");
            } else {
                throw new Error("Invoice yaratishda xato");
            }
        } catch (err) {
            notify.error("Utilizatsiyani boshlashda xato: " + (err?.message || err));
        } finally {
    ...
    // normalize (ensure stock_quantity available)
    function normalizeIncomingItem(raw) {
        ...
        return {
            ...
            price: Number(raw.price) || 0, // price comes from stock
            product: productObj,
    ...
    function addItemToMixData(raw) {
        ...
        // For disposal we always allow adding (we assume selecting from stock of current location)
    ...
    const total = useMemo(() => {
        ...
        return mixData.reduce((sum, it) => sum + safeNum(it.price) * safeNum(it.quantity), 0);
    }, [mixData]);
    ...
    function handleUpdateQuantity(index, value) {
        // allow empty string, numeric >0 or "0" : clamp to stock_quantity
        const item = mixData[index];
        ...
        try {
            ...
            const payload = {
                list: mixData.map((it) => ({
                    ...
                    is_new_batch: false,
                    batch: it.batch,
                    // optionally: disposal flag if backend expects
                    // disposal: true
                })),
            };
    ...
    const openModal = () => {
        ...
        const value_spaces = mixData.filter((item) => item.quantity === "" || Number(item.quantity) === 0);
        if (value_spaces.length > 0) {
            value_spaces.forEach((err) => {
                notify.warning(err.product?.name + " tovar uchun Miqdor kiriting");
            });
            return;
        }
    ...
    const handlePrint = async () => {
      ...
      `;
            ...
            printWindow.document.write(`<html><head><title>UTILIZATSIYA HUJJATI</title>${style}</head><body>${content}</body></html>`);
    ...
    return (
        <section className="relative w-full min-h-screen bg-white overflow-hidden">
            <div className="fixed top-0 right-0 w-full h-[68px] backdrop-blur-[3px] bg-amber-50/80 border-b border-amber-100 shadow flex items-center justify-center text-xl font-semibold z-30 text-amber-800">
                Utilizatsiya (Disposal)
            </div>
                ...
                <div className="flex items-center justify-between p-2 border-b border-gray-200">
                    ...
                    {(isMedium || isWide) && (
                        <div className="flex gap-2">
                            <button onClick={() => setViewMode("category")} className={`flex items-center gap-1 px-2 py-1 rounded-lg transition ${viewMode === "category" ? "bg-amber-700 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`} aria-pressed={viewMode === "category"}>
                                <Tags size={18} />
                                {isWide && <span className="text-sm">Category</span>}
                            </button>
                            <button onClick={() => setViewMode("product")} className={`flex items-center gap-1 px-2 py-1 rounded-lg transition ${viewMode === "product" ? "bg-amber-700 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`} aria-pressed={viewMode === "product"}>
                                <Package size={18} />
                    ...
                    <div className={`overflow-y-auto p-3 grid gap-3 overflow-x-scroll grid-cols-[repeat(auto-fill,minmax(auto,1fr))]`}>
                        {viewMode === "category" ? (
                            groupLoading ? (
                                ...
                                categories.map((cat) => (
                                    <button key={cat.id} onClick={() => handleCategoryClick(cat.id)} className={`cursor-pointer border rounded-xl shadow-sm hover:shadow-md p-3 text-left ${selectedCategory === cat.id ? "border-amber-500 bg-amber-50" : "border-gray-300 bg-white"}`}>
                                ...
                                .map((prod) => (
                                    <button key={prod.id || prod.stock_id || prod.product?.id || generateId()} onClick={() => onSidebarProductClick(prod)} className="active:scale-[0.99]">
                                        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md p-3 min-w-[180px] transition">
                                            <div className="p-1 bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200">
            ...
            <div className={`transition-all duration-500 ease-in-out pt-[68px] ${sidebarMode === 0 ? "ml-[70px]" : sidebarMode === 1 ? "ml-[25%]" : "ml-[33.3%]"} p-6`}>
                <div className="bg-amber-50 rounded-2xl min-h-[calc(100vh-68px)] p-4 flex flex-col gap-4">
                    {/* HEAD */}
                    {!invoiceStarted?.[mode] ? (
                        <div className="h-[65px] bg-white rounded-lg flex items-center gap-4 px-3 shadow-sm border-l-4 border-amber-500">
                            <div className="flex items-center gap-2">
                                <div className="text-sm text-gray-500">Utilizatsiya qabul manzili</div>
                                {locationsLoading ? (
                                    <div className="flex items-center gap-2"><Spinner /> Tekshirilmoqda...</div>
                                ) : (
                                    <div className="font-medium">{disposalLocation?.name || "Disposal location topilmadi"}</div>
                                )}
                            </div>

                            <div className="ml-auto">
                                <button disabled={createInvoiceLoading} onClick={startInvoice} className={`${touchBtn} flex items-center gap-2 bg-amber-700 text-white rounded hover:opacity-95`} aria-label="Start disposal">
                                    ...
                                    Utilizatsiyani boshlash
                                </button>
                            </div>
                        </div>
                    ) : (
                        <div className="h-[65px] bg-white rounded-lg flex items-center gap-3 px-3 shadow-sm border-l-4 border-amber-500">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="ml-auto flex items-center gap-2">
                                <button onClick={() => { setBarcodeEnabled((s) => !s); setBarcodeInput(""); }} className={`flex items-center gap-2 px-3 py-2 rounded ${barcodeEnabled ? "bg-amber-700 text-white animate-[pulse_1.5s_infinite]" : "bg-gray-200 text-gray-800"}`} aria-pressed={barcodeEnabled} aria-label="Toggle barcode input">
                    ...
                    {invoiceStarted?.[mode] && (
                        <>
                            <div className="bg-white rounded-lg p-3 shadow-sm flex items-center gap-6 border-l-4 border-amber-300">
                                <div>
                                ...
                                <div>
                                    <div className="text-xs text-gray-500">Qabul (utilizatsiya)</div>
                                    <div className="font-medium">{invoiceMeta?.[mode]?.receiver || disposalLocation?.name || "Disposal"}</div>
                                </div>
                                <div>
                                ...
                                <table className="min-w-full text-sm">
                                    ...
                                    <tbody>
                                        {(!mixData || mixData.length === 0) ? (
                                            <tr><td colSpan={9} className="p-4 text-center text-gray-400">Mahsulotlar mavjud emas</td></tr>
                                        ) : mixData.map((it, idx) => {
                                            ...
                                            return (
                                                <tr key={it.id || idx} className="border-b">
                                                    ...
                                                    <td className="p-2 align-center w-[140px]">
                                                        {/* Price is not editable for disposal */}
                                                        <div className="px-2 py-1 rounded bg-gray-50 border text-gray-800">{Number(it.price || 0).toLocaleString()}</div>
                                                    </td>
                                                    ...
                                                    <td className="p-2 aligin-center text-amber-900">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex items-center gap-2">
                                    <button onClick={openModal} className={`${touchBtn} flex items-center gap-2 bg-amber-700 text-white text-[16px] rounded hover:opacity-95`}>
                                        <CheckSquare size={22} />
                                        Yakunlash (Utilizatsiya)
                                    </button>
                                </div>
                            </div>
                        </>
                    ...
                    <div className="bg-white rounded shadow-lg w-[210mm] max-w-full max-h-[95vh] overflow-auto p-6" aria-live="polite">
                        <div id="modal_window" ref={modalContentRef}>
                            <div style={{ width: "100%", boxSizing: "border-box" }}>
                                <h1 id="modal-title" className="text-center text-lg font-bold">UTILIZATSIYA HUJJATI</h1>
                                <div className="meta">
                                    <div><strong>Jo'natuvchi:</strong> {invoiceMeta?.[mode]?.sender || getLocationNameById(userLId)}</div>
                                    <div><strong>Qabul (utilizatsiya):</strong> {invoiceMeta?.[mode]?.receiver || disposalLocation?.name || "—"}</div>
                                ...
                                <div style={{ marginTop: 20 }}>
                                    ...
                                    <div style={{ marginTop: 8 }}>Qabul (utilizatsiya) imzo: ______________________</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-4 flex justify-end gap-2">
                            ...
                            <button onClick={handleModalSave} disabled={saving} className="px-4 py-2 rounded bg-amber-700 text-white disabled:opacity-60">
